...
/** Turn any URL into a /fetch?url= proxy link */
function proxyHref(absUrl) {
  return `/fetch?url=${encodeURIComponent(absUrl)}`;
}
...
function rewriteHtml(html, pageUrl) {
  ...
  html = html.replace(
    ...
    (m, attr, q, val) => {
      const rewritten = val.replace(/(https?:\/\/[^\s,]+)/gi, u => proxyHref(u));
...
app.get('/fetch', async (req, res) => {
  ...
  try {
    ...
    if (/text\/html/i.test(contentType)) {
      ...
      const rewritten = rewriteHtml(htmlText, targetUrl);
