...
function getBackendBase(req) {
  // Prefer explicit env var so deploy URL is always correct
...
(function(){
  ...
  /* ── Signal ready for extension injection ───────────────────────────────── */
...
app.get('/health', (_req, res) => res.json({ status: 'ok', version: '2.2', ts: Date.now() }));
...
app.get('/fetch', async (req, res) => {
  ...
  try {
    ...
    res.setHeader('Access-Control-Allow-Origin', '*');
    // Remove content-length since rewritten HTML will be a different size
...
app.listen(PORT, () => {
  console.log(`EP Proxy server v2.2 running on http://localhost:${PORT}`);
});
